...
module ConvolutionHaskell where
...
convolve dimension filter input = foldl (+) 0 convolve'
	where
		convolve' = zipWith doOneShift [-shiftBounds .. shiftBounds] filter
		-- Convert the input to GPU data representation.
		pyInput = pySet input
		-- Get the bounds of the convolution filter.
		shiftBounds = (length filter - 1) `div` 2
		-- Shift once and multiply.
		doOneShift sBy mBy = pyShiftExtend (sBy : dimension) pyInput * pyConst mBy
