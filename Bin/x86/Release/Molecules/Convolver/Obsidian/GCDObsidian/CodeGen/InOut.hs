...
tryGetName (Array (Pull f) _) = case f (Literal 0) of
	Index (n, _) -> Just n
	otherwise    -> Nothing
...
newInOut name t s = do 
  ...
  --if ((newname, t, s) `elem` ins) then put (i, ins) 
  --else put (i + 1, (newname, t, s) : ins) 
  put (i+1,(newname,t,s):ins) 
--  put (i,(newname,t,s):ins) 
  return newname

existingInOut name t s = do 
  (i, ins) <- get
  if ((name, t, s) `elem` ins) then put (i, ins) 
  else put (i, (name, t, s) : ins) 
  return name
...
instance Scalar a => InOut (Array Pull (Exp a)) where
  createInputs arr  = do 
    name <- case tryGetName arr of
		Nothing -> newInOut "input" (cTypeOfArray arr) (len arr)
		Just nm -> existingInOut nm (cTypeOfArray arr) (len arr)
